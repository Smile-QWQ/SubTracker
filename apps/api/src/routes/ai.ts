...
export async function aiRoutes(app: FastifyInstance) {
  app.get('/ai/summary/dashboard', async (_request, reply) => {
    try {
      ...
      return sendError(
        reply,
        400,
        'ai_summary_fetch_failed',
        error instanceof Error ? error.message : 'api.errors.ai.summaryFetchFailed'
      )
    }
  })
  ...
  app.post('/ai/test', async (request, reply) => {
    try {
      ...
      return sendError(reply, 400, 'ai_test_failed', error instanceof Error ? error.message : 'api.errors.ai.connectionTestFailed')
    }
  })
  ...
  app.post('/ai/test-vision', async (request, reply) => {
    try {
      ...
      return sendError(reply, 400, 'ai_vision_test_failed', error instanceof Error ? error.message : 'api.errors.ai.visionTestFailed')
    }
  })
  ...
  app.post('/ai/recognize-subscription', async (request, reply) => {
    try {
      ...
      return sendError(reply, 400, 'ai_recognition_failed', error instanceof Error ? error.message : 'api.errors.ai.recognitionFailed')
    }
  })
}
