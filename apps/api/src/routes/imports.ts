...
export async function importRoutes(app: FastifyInstance) {
  app.post('/import/wallos/inspect', async (request, reply) => {
    ...
    try {
      ...
      return sendError(reply, 400, 'wallos_inspect_failed', error instanceof Error ? error.message : 'api.errors.imports.wallosInspectFailed')
    }
  })
  ...
  app.post('/import/wallos/commit', async (request, reply) => {
    ...
    try {
      ...
      return sendError(reply, 400, 'wallos_commit_failed', error instanceof Error ? error.message : 'api.errors.imports.wallosCommitFailed')
    }
  })
