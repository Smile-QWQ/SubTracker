    ...
    <n-card style="margin-bottom: 12px">
      <n-space vertical :size="12" style="width: 100%">
        <div class="filters-grid">
          <n-input v-model:value="filters.q" :placeholder="t('subscriptions.page.searchPlaceholder')" clearable @keyup.enter="loadSubscriptions" />
          <n-select v-model:value="filters.status" clearable :placeholder="t('subscriptions.page.statusPlaceholder')" :options="statusOptions" />
    ...
    <n-card>
      ...
      <n-space v-if="batchMode" justify="space-between" align="center" style="margin-bottom: 12px" wrap>
        <n-space align="center" wrap>
          ...
          <n-button size="small" :disabled="selectedCount === 0" @click="clearSelectedSubscriptions">{{ t('subscriptions.page.clearSelection') }}</n-button>
        </n-space>
        <n-space wrap>
          <n-button size="small" type="primary" ghost :disabled="selectedCount === 0" @click="runBatchRenew">{{ t('subscriptions.actions.batchRenew') }}</n-button>
          <n-button size="small" :disabled="selectedCount === 0" @click="runBatchSetStatus('active')">{{ t('subscriptions.actions.setActive') }}</n-button>
          <n-button size="small" :disabled="selectedCount === 0" @click="runBatchSetStatus('paused')">{{ t('subscriptions.actions.setPaused') }}</n-button>
          <n-button size="small" type="warning" ghost :disabled="selectedCount === 0" @click="runBatchSetStatus('cancelled')">{{ t('subscriptions.actions.setCancelled') }}</n-button>
          <n-button size="small" type="error" ghost :disabled="!canBatchDelete" @click="runBatchDelete">{{ t('subscriptions.actions.batchDelete') }}</n-button>
        </n-space>
      </n-space>
        ...
        <n-card v-for="item in orderedSubscriptions" :key="item.id" size="small" class="mobile-subscription-card">
            ...
            <div class="mobile-subscription-card__title-wrap">
              ...
              <div class="mobile-subscription-card__title-group">
                ...
                <div class="mobile-subscription-card__meta">
                  {{ item.currency }} {{ Number(item.amount).toFixed(2) }} · {{ t('subscriptions.values.interval', { count: item.billingIntervalCount, unit: unitLabel(item.billingIntervalUnit) }) }}
                </div>
              </div>
            </div>
          ...
          <div class="mobile-subscription-card__rows">
            <div class="mobile-subscription-card__row">
              <span>{{ t('subscriptions.labels.nextRenewal') }}</span>
              <span>{{ formatDate(item.nextRenewalDate) }}</span>
            </div>
            <div class="mobile-subscription-card__row">
              <span>{{ t('subscriptions.labels.autoRenew') }}</span>
          ...
          <n-space v-if="!batchMode" wrap style="margin-top: 12px">
            ...
            <template v-if="item.status === 'active'">
              <n-popconfirm :positive-text="t('common.actions.confirm')" :negative-text="t('common.actions.cancel')" @positive-click="pause(item.id)">
                <template #trigger>
...
const mainColumns = [
  ...
  },
  {
    ...
    colSpan: (row: SubscriptionTableRow) => (row.__rowType === 'note' ? 0 : 1),
    render: (row: SubscriptionTableRow) =>
      row.__rowType === 'note' ? null : t('subscriptions.values.interval', { count: row.billingIntervalCount, unit: unitLabel(row.billingIntervalUnit) })
  },
  {
      ...
      const statusActions =
        row.status === 'active'
          ? [
              h(
                NPopconfirm,
                { positiveText: t('common.actions.confirm'), negativeText: t('common.actions.cancel'), onPositiveClick: () => void pause(row.id) },
                {
              ...
              h(
                NPopconfirm,
                { positiveText: t('common.actions.confirm'), negativeText: t('common.actions.cancel'), onPositiveClick: () => void cancel(row.id) },
                {
          ...
          : row.status === 'paused'
            ? [
                h(
                  NPopconfirm,
                  { positiveText: t('subscriptions.actions.resume'), negativeText: t('common.actions.cancel'), onPositiveClick: () => void resume(row.id) },
                  {
  ...
  try {
    ...
    message.error(
      t('subscriptions.messages.subscriptionSaveFailed', {
        message: error instanceof Error ? error.message : t('statistics.fallback.unknown')
      })
    )
  } finally {
...
async function runBatchSetStatus(status: BatchSettableStatus) {
  ...
  summarizeBatchResult(`${t('subscriptions.page.batchMode')} ${statusLabel}`, result)
  await refetchCurrentSubscriptions()
  await refreshOpenDetailIfNeeded(ids)
}
...
async function runBatchDelete() {
  ...
  const activeDeleteBlockedMessage = getMessage(getAppLocale(), 'api.errors.subscriptions.activeDeleteBlocked')
  const skippedActiveCount = result.failures.filter((item) => item.message === activeDeleteBlockedMessage).length
...
async function removeSubscription(id: string, name: string) {
  try {
    ...
    message.error(error instanceof Error ? error.message : t('subscriptions.actions.batchDelete'))
  }
}
...
function statusText(status: Subscription['status']) {
  return {
    active: getMessage(getAppLocale(), 'common.status.active'),
    paused: getMessage(getAppLocale(), 'common.status.paused'),
    cancelled: getMessage(getAppLocale(), 'common.status.cancelled'),
    expired: getMessage(getAppLocale(), 'common.status.expired')
  }[status]
}
