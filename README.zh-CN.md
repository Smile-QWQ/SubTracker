...
- **通知能力**：支持 Webhook、SMTP / Resend 邮件、PushPlus、Telegram Bot、Server 酱、Gotify、Bark、NotifyX 和 Apprise。
- **Logo 能力**：支持上传（R2）、远程引用、网络搜索。
...
- **不使用 KV**。
- 邮件通知支持 **SMTP / Resend**，但 **SMTP 不支持 25 端口**。
- 不再提供标签月预算和独立预算页。
...
- [Hono](https://hono.dev/) 与 [Prisma](https://www.prisma.io/) —— 提供 Worker 后端与数据访问能力
